...
it('renders recordRow correctly with showing all columns', () => {
    ...
    const { getByTestId, queryByTestId, queryAllByTestId } = render(<RecordRow {...recordRowProps} />,  {
        container: document.body.appendChild(tbody),
    });
    expect(getByTestId('recordRow'));
    // campaign cell should show the data
    expect(queryByTestId('dataCellCampaignId')).toHaveTextContent(recordRowProps.rowData.groups.Campaign.metadata.name);
    // graph cells should show the data
    for (let [index, ] of recordRowProps.dataTableGraphRecords.entries()) {
...
it('renders recordRow correctly when campaign id cells are hidden', () => {
    const tbody = document.createElement('tbody');
    const { queryByTestId, queryAllByTestId } = render(<RecordRow {...recordRowProps} isCampaignColumnOpen={false} />,  {
        container: document.body.appendChild(tbody),
    });
    // campaign cell should not show the data
    expect(queryByTestId('dataCellCampaignId')).not.toHaveTextContent(recordRowProps.rowData.groups.Campaign.metadata.name);
    // graph cells should show the data
    for (let [index] of recordRowProps.dataTableGraphRecords.entries()) {
        expect(queryAllByTestId('dataCellGraph')[index].firstChild);
    }
...
it('renders recordRow correctly when graph cells are hidden', () => {
    const tbody = document.createElement('tbody');
    const { queryByTestId, queryAllByTestId } = render(<RecordRow {...recordRowProps} isGraphColumnsOpenArray={new Array(recordRowProps.dataTableGraphRecords.length).fill(false)}  />,  {
        container: document.body.appendChild(tbody),
    });
    // campaign cell should show the data
    expect(queryByTestId('dataCellCampaignId')).toHaveTextContent(recordRowProps.rowData.groups.Campaign.metadata.name);
...
it('matches snapshot', () => {
    const tbody = document.createElement('tbody');
    const tree = renderer.create(<RecordRow {...recordRowProps} />,  {
        container: document.body.appendChild(tbody),
    }).toJSON();
