...
jest.mock("highcharts-react-official", () => ({ options }) => {
...
it("renders dataCellGraph correctly", () => {
  ...
  const { getByTestId } = render(
    <DataCellGraph {...dataCellGraphProps} isOpen={false} />,
    {
  ...
  const total = dataCellGraphProps.rowData.trend.reduce(
    (totalCollect, dayData) =>
      totalCollect + dayData[dataCellGraphProps.columnObject.key],
    0
  )
...
it("matches snapshot", () => {
  const tree = renderer
    .create(<DataCellGraph {...dataCellGraphProps} />)
    .toJSON()
